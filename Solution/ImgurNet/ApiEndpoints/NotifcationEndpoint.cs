...
namespace ImgurNet.ApiEndpoints
{
	public class NotifcationEndpoint : BaseEndpoint
	{
		...
		public async Task<ImgurResponse<Notification<object>>> GetNotificationAsync(string notificationId)
		{
			...
			return await Request.SubmitImgurRequestAsync<Notification<object>>(Request.HttpMethod.Get, string.Format(NotificationUrl, notificationId), Imgur.Authentication);
		}
		...
		public async Task<ImgurResponse<Boolean>> SetNotificationViewed(string notificationId)
		{
			...
			return await Request.SubmitImgurRequestAsync<Boolean>(Request.HttpMethod.Put, string.Format(NotificationUrl, notificationId), Imgur.Authentication);
		}
	}
}
